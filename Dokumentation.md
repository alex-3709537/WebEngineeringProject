...
Die beiden Funktionen rufen jeweils die Funktion `connectAndQuery2` auf. In dieser Funktion wird das `mysql2`-Paket genutzt, damit asynchrone Requests an den SQL-Server gesendet werden können. Zusätzlich bietet das Paket die Funktion `connection.execute(query, data)`, in welcher die Query und die Parameter getrennt übergeben werden. Im Query werden die Stellen an denen ein Parameter steht mit einem Fragezeichen ersetzt. Diese Funktion ist nützlich um Sql Injections vor zu beugen.
